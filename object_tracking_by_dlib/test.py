...
def main():
    ...
    try:
        while True:
            ...
            # dlib记录坐标位置
            if len(points) == 2:
                object_rectangle = dlib.rectangle(points[0][0], points[0][1], points[1][0], points[1][1])

                tracking_state = mouse_event_judge(frame, object_rectangle, tracker, tracking_state)
...
def mouse_event_judge(frame, object_rectangle, tracker, tracking_state):
    try:
        global points
        # 鼠标事件判断，根据按键：'1', '2', 'q'
        ...
        if key == ord('1'):
            if len(points) == 2:
                tracker.start_track(frame, object_rectangle)
                tracking_state = True
                points = []
        if key == ord('2'):
            tracking_state = False
